...
class AVLStabloMapa : public Mapa<TipKljuca,TipVrijednosti> {
    private:
        ...
        void brisi(AVLCvor<TipKljuca,TipVrijednosti>* cvor);
    public:
...
template <typename TipKljuca, typename TipVrijednosti>
void AVLStabloMapa<TipKljuca,TipVrijednosti>::brisi(AVLCvor<TipKljuca,TipVrijednosti>* cvor) {
    if(cvor!=nullptr) {
        brisi(cvor->lijevi);
        brisi(cvor->desni);
        delete cvor;
...
int main() {
    ...
    clock_t t4=clock();
    std::cout << "Vrijeme unosa 7500 elemenata za binarno stablo je " << (t2-t1)/(CLOCKS_PER_SEC/1000) << " ms, a za AVL stablo " << (t4-t3)/(CLOCKS_PER_SEC/1000) << " ms." << std::endl;
    ...
    t4=clock();
    std::cout << "Vrijeme brisanja 7500 elemenata za binarno stablo je " << (t2-t1)/(CLOCKS_PER_SEC/1000) << " ms, a za AVL stablo " << (t4-t3)/(CLOCKS_PER_SEC/1000) << " ms.";
}
